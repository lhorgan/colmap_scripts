...
def get_corresponding_poses(svin_path, svin_from_colmap_path):
        ...
        with open(svin_from_colmap_path) as f:
            for line in f:
                ...
                image_name_to_svin_pose[image_name] = np.array([tx, ty, tz, qx, qy, qz, qw])#pose_matrix
        ...
        with open(svin_path) as f:
            for line in f:
                ...
                #pose_matrix = create_pose_matrix(tx, ty, tz, qx, qy, qz, qw)
                image_name_to_colmap_pose[image_name] = np.array([tx, ty, tz, qx, qy, qz, qw])#pose_matrix
...
def compute_alignment_stats(scene):
    ...
    svin_centers = np.array([[p[0], p[1], p[2]] for p in svin_poses])
    colmap_centers = np.array([[p[0], p[1], p[2]] for p in colmap_poses])
...
def go():
    ...
    for scene in cube_dirs:
        #try:
            #compute_alignment_stats_per_cam(scene)
