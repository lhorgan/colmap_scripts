...
def compute_alignment_stats(scene, model):
    data_path = "/home/luke/Documents/titanic/spheres"
    #scene = "bin_0"
    svin_scaffold_path = "/home/luke/Documents/titanic/svin_scaffold.txt"
    svin_from_colmap_inv_path = f"{data_path}/{scene}/sparse/{model}/svin_from_colmap_inv.txt"
    ...
    print(f"Computing stats for {scene}_{model}, with {len(svin_centers)} centers.")
...
def go():
    ...
    for scene in cube_dirs:
        for model in os.listdir(f"{data_path}/{scene}/sparse"):
            try:
                errors = compute_alignment_stats(scene, model)
